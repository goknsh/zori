    ...
    switch ($_SERVER["REQUEST_METHOD"]) {
        case ("POST" || "GET"):
            ...
            if (isset($body["url"]) || isset($body["uid"])) {
                    ...
                    foreach ([1, 2, 3] as $num) {
                        if ($xpath->query("//html/body/div[$num]")->item(0)->previousSibling->textContent === strtoupper("poetry") . " SUBMISSIONS") {
                            ...
                            foreach ($xpath->query("//html/body/div[$num]/div/table/tr[contains(@class, 'root-story')]") as $key => $singles) {
                                $response["singles"]["poetry"][$key]["title"] = $singles->childNodes->item(0)->childNodes->item(0)->textContent;
                                $response["singles"]["poetry"][$key]["link"] = $singles->childNodes->item(0)->childNodes->item(0)->getAttribute("href");
                                $response["singles"]["poetry"][$key]["rating"] = (double) str_replace(")", "", str_replace("&nbsp;(", "", htmlentities($singles->childNodes->item(0)->childNodes->item(1)->textContent)));
                                $response["singles"]["poetry"][$key]["description"] = preg_replace("/\n                   /", "", $singles->childNodes->item(1)->textContent);
                                $response["singles"]["poetry"][$key]["category"] = $singles->childNodes->item(2)->childNodes->item(1)->textContent;
                                $response["singles"]["poetry"][$key]["date"] = $singles->childNodes->item(3)->textContent;
                            }
                        }
                    }
                }
                ...
                $totalStories = count($response["singles"]["poetry"]);

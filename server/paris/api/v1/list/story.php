    ...
    switch ($_SERVER["REQUEST_METHOD"]) {
        case ("POST" || "GET"):
            ...
            if (isset($body["url"]) || isset($body["uid"])) {
                    ...
                    foreach ([1, 2, 3] as $num) {
                        if ($xpath->query("//html/body/div[$num]")->item(0)->previousSibling->textContent === strtoupper("story") . " SUBMISSIONS") {
                            foreach ($xpath->query("//html/body/div[$num]/div/table/tr[contains(@class, 'ser-ttl')]") as $key => $seriesTitle) {
                                ...
                                $response["series"]["story"][$key]["title"] = str_replace(": $match[0]", "", $seriesTitle->textContent);
                                $response["series"]["story"][$key]["number"] = (int) explode(" ", $match[0])[0];
                                $response["series"]["story"][$key]["parts"] = array();
                                if ($key === 0) {
                                    $iMax = 0;
                                } elseif ($key !== 0) {
                                    $iMax = $iMax + $response["series"]["story"][$key-1]["number"];
                                }
                                for ($i=$iMax; $i < $iMax+$response["series"]["story"][$key]["number"]; $i++) {
                                    $partRow = $xpath->query("//html/body/div[$num]/div/table/tr[contains(@class, 'sl')]");
                                    if ($partRow->item(0) !== null) {
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["title"] = $partRow->item($i)->childNodes->item(0)->childNodes->item(0)->textContent;
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["link"] = $partRow->item($i)->childNodes->item(0)->childNodes->item(0)->getAttribute("href");
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["rating"] = (double) str_replace(")", "", str_replace(" (", "", $partRow->item($i)->childNodes->item(0)->childNodes->item(1)->textContent));
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["description"] = preg_replace("/\n                   /", "", $partRow->item($i)->childNodes->item(1)->textContent);
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["category"] = $partRow->item($i)->childNodes->item(2)->childNodes->item(1)->textContent;
                                        $response["series"]["story"][$key]["parts"][$i-$iMax]["date"] = $partRow->item($i)->childNodes->item(3)->textContent;
                                    }
                                }
                            }
                ...
                foreach ($response["series"]["story"] as $series) {
                ...
                if (isset($body["sortBy"])) {
                    ...
                    if ($body["sortBy"] === "key") {
                        ...
                        if ($body["sortType"] === "ksort" || $body["sortType"] === "alpha" || $body["sortType"] === "alphabetize" || $body["sortType"] === "alphabetical") {
                            ...
                            if ($body["sortDepth"] === "full") {
                                foreach ($response["series"]["story"] as $key => $series) {
                                    ksort($response["series"]["story"][$key]);
                                    foreach ($series["parts"] as $partKey => $part) {
                                        ksort($response["series"]["story"][$key]["parts"][$partKey]);
                                    }
                                }
                            }
                        }
                        ...
                        if ($body["sortType"] === "krsort" || $body["sortType"] === "ralpha" || $body["sortType"] === "ralphabetize" || $body["sortType"] === "ralphabetical") {
                            ...
                            if ($body["sortDepth"] === "full") {
                                foreach ($response["series"]["story"] as $key => $series) {
                                    krsort($response["series"]["story"][$key]);
                                    foreach ($series["parts"] as $partKey => $part) {
                                        krsort($response["series"]["story"][$key]["parts"][$partKey]);
                                    }
                                }
                            }
                        }
                    ...
                    if ($body["sortBy"] === "val" || $body["sortBy"] === "value") {
                        ...
                        if ($body["sortType"] === "asort" || $body["sortType"] === "alpha" || $body["sortType"] === "alphabetize" || $body["sortType"] === "alphabetical") {
                            ...
                            if ($body["sortDepth"] === "full") {
                                foreach ($response["series"]["story"] as $key => $series) {
                                    asort($response["series"]["story"][$key]);
                                    foreach ($series["parts"] as $partKey => $part) {
                                        asort($response["series"]["story"][$key]["parts"][$partKey]);
                                    }
                                }
                            }
                        }
                        ...
                        if ($body["sortType"] === "arsort" || $body["sortType"] === "ralpha" || $body["sortType"] === "ralphabetize" || $body["sortType"] === "ralphabetical") {
                            ...
                            if ($body["sortDepth"] === "full") {
                                foreach ($response["series"]["story"] as $key => $series) {
                                    arsort($response["series"]["story"][$key]);
                                    foreach ($series["parts"] as $partKey => $part) {
                                        arsort($response["series"]["story"][$key]["parts"][$partKey]);
                                    }
                                }
                            }
                        }
